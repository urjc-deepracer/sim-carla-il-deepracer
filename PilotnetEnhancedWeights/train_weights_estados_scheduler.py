...
def compute_estado_weights_from_dataset(dataset, device, steer_threshold=0.20, eps=1e-8):
    ...
    # dataset.labels es lista de [steer, throttle] según tu dataset actual
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Modo de muestreo: none | weighted"
    )
...
def main():
    ...
    if args.sampling_mode == "weighted":
        ...
        print("[INFO] Usando WeightedRandomSampler")

        dataset_for_sampler = train_dataset

        all_estados = []
        for i in range(len(dataset_for_sampler)):
            _, _, estado = dataset_for_sampler[i]
            all_estados.append(int(estado))

        all_estados = torch.tensor(all_estados)
        idx = (all_estados - 1).clamp(0, 2)

        sample_weights = w_global[idx].cpu()
        ...
        print("[INFO] Muestreo normal con shuffle=True")

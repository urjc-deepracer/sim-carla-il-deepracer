...
def balanced_mse_batch(pred, target, estados, debug=False):
    
    # Hace batch balancing en la loss:
    # Calcula MSE por estado y promedia los estados.
    # No depende de pesos globales.

    loss_per_state = []
    estados = estados.long()
    ...
    for c in [1, 2, 3]:
        ...
        if mask.sum() > 0:
            mse_c = ((pred[mask] - target[mask]) ** 2).mean()
            loss_per_state.append(mse_c)

            if debug:
                print(f"Estado {c}: {mask.sum().item()} samples | MSE = {mse_c.item():.6f}")

    if len(loss_per_state) == 0:
        return torch.tensor(0.0, device=pred.device)

    loss = torch.stack(loss_per_state).mean()

    if debug:
        print("Loss final balanceada:", loss.item())
        print("--------------------------\n")
...
def compute_estado_weights_from_dataset(dataset, device, steer_threshold=0.20, eps=1e-8):
    ...
    # dataset.labels es lista de [steer, throttle]
...
def parse_args():
    ...
    p.add_argument("--online_label_aug", action="store_true")
    p.add_argument("--online_aug_prob", type=float, default=0.3)
    p.add_argument("--online_steer_std", type=float, default=0.01)
    p.add_argument("--online_throttle_std", type=float, default=0.01)
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        mode="min",            # minimizar val_mse
        factor=0.5,            # reduce LR a la mitad
        patience=8,            # espera 10 epochs sin mejora
        verbose=True
    )
    ...
    for epoch in range(last_epoch, args.num_epochs):
        ...
        for i, (images, labels, estados) in enumerate(train_loader):
            ...
            if args.online_label_aug:
                ...
                mask = torch.rand(labels.size(0), device=device) < args.online_aug_prob

                if mask.any():
                    noise_steer = torch.randn(mask.sum(), device=device) * args.online_steer_std
                    noise_throttle = torch.randn(mask.sum(), device=device) * args.online_throttle_std

                    labels_aug = labels.clone()

                    labels_aug[mask, 0] += noise_steer
                    labels_aug[mask, 1] += noise_throttle

                    # clipping
                    labels_aug[:, 0] = torch.clamp(labels_aug[:, 0], -1.0, 1.0)
        ...
        print(f"[LR] Epoch {epoch+1}: learning rate = {current_lr:.8f}")

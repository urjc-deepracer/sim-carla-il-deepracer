...
def main():
    ...
    ax_mse.plot(dist, linewidth=1.2, label="Distancia euclídea por punto")
    ax_mse.fill_between(range(len(dist)),
                    dist,
                    color="#3D5A80",
                    alpha=0.25)

    ax_mse.set_title("Error espacial por punto (distancia euclídea IA vs Piloto Experto)",
                 fontsize=13, weight="bold")

    ax_mse.set_xlabel("Muestras piloto experto")
    ax_mse.set_ylabel("Distancia a muestras IA (m)")
